...
tumble、pan、zoom。新副本以源网格世界矩阵作为局部基准矩阵,进入 content 的稳定预览空间,
避免 Maya 的“保持世界变换”错误继承远距隐藏偏移。窗口同时保留可视化控件状态与同名
...
让副本在窗口里呈现目标引擎视口中的朝向。纯坐标基转换不会改变物体的物理姿态,因此预览模型保持
Maya 原姿态,只把目标引擎坐标轴嵌入 Maya 固定的 Y-up 视口。`core/coord_convention.py` 把每套
...
隔离视口还会在原点创建贯穿正负方向的 X/Y/Z 细线轴(X 红、Y 绿、Z 蓝),长度随资产与 LOD 调整,
方向使用同一目标显示基。界面的“显示原点坐标轴”默认开启;曲线变换被锁定,不会误改预览内容。
...
不到。该镜像只用于表达目标坐标基,不作用于预览网格,因此不需要反转网格法线或绕序。
`MAYA_TO_UE` 矩阵集中放在一处常量,若导出器设置(轴系统 / Force Front)不同只需改这一处。
